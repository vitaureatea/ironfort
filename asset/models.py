...
class Asset(models.Model):
    ...
    c_time = models.DateTimeField(auto_now_add=True, verbose_name='登记日期')
...
class Server(models.Model):
    ...
    asset = models.OneToOneField('Asset', blank=True, null=True,on_delete=models.SET_NULL)  # 非常关键的一对一关联！
...
class SecurityDevice(models.Model):
    ...
    asset = models.OneToOneField('Asset', blank=True, null=True,on_delete=models.SET_NULL)
...
class CPU(models.Model):
    ...
    asset = models.OneToOneField('Asset', blank=True, null=True,on_delete=models.SET_NULL)  # 设备上的cpu肯定都是一样的，所以不需要建立多个cpu数据，一条就可以，因此使用一对一。
...
class RAM(models.Model):
    ...
    asset = models.ForeignKey('Asset', blank=True, null=True,on_delete=models.SET_NULL)  # 只能通过外键关联Asset。否则不能同时关联服务器、网络设备等等。
...
class Disk(models.Model):
    ...
    sn = models.CharField('硬盘SN号', max_length=128, blank=True, null=True)
...
class NIC(models.Model):
    ...
    asset = models.ForeignKey('Asset', blank=True, null=True,on_delete=models.SET_NULL)  # 注意要用外键
    ...
    mac = models.CharField('MAC地址', max_length=64,blank=True, null=True)  # 虚拟机有可能会出现同样的mac地址
